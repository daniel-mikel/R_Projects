...
DE_Polling <- read.csv("/home/dan/R Projects/Elections/Data/German Polling Data Dummy.csv")
glimpse(DE_Polling)
...
qtm(DEmap, fill="Largest", fill.palette = colors1 )
  ...
  #broken =(
tm_shape(DEmap) +
  tm_fill("Largest", title="Party with Greatest Support", palette = colors1) +
  tm_borders(alpha=.7) +
  tm_text("Name_1", size=0.5) +
  tm_legend(position=c(.02, .02), frame=TRUE) 

tm_shape(DEmap) +
  tm_fill("Largest", title="Title", palette = colors2) +
  tm_borders(alpha=.7) +
  tm_text("Name_1", size=0.5) +
  tm_legend(position=c(.02, .02), frame=TRUE) 
...
plot(DEmap)
plot(DEmap, "Largest")
plot(DEmap, DEmap$Largest, legend = FALSE, col = colors1)

  #the legend works...
plot(DEmap)
legend("bottomleft", legend = c("CDU.CSU", "SPD", "GRÜNE", "FDP", "Die Linke", "AfD", "Sonstige_Total"), fill = colors1)
...
#[Section Title] Creating the 3d map with popup menues
...
Sonstige_TotalPalette <- colorNumeric(palette = "Pinks", domain = c(minpct, maxpct))

largestPalette <- colorFactor(palette=c("#2a202b", "#e41a1c", "#33c47f", "#edbe04",  "#680d0d", "#3c1faf", "#e530e5"), 
...
DELayeredmap <- leaflet(DEmap) %>%
  ...
  addLayersControl(
    baseGroups=c("CDU.CSU", "SPD", "GRÜNE", "FDP", "Die Linke", "AfD", "Sonstige_Total"),
    position = "bottomleft",
